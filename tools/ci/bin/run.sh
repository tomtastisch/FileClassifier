...
run_consumer_smoke() {
  ...
  run_or_fail "CI-SMOKE-001" "Restore consumer sample from package" dotnet restore "${consumer_project}" --configfile "${consumer_nuget_config}" -p:PortableConsumerPackageVersion="${package_version}" -p:RestoreLockedMode=false -v minimal
...
run_package_backed_tests() {
  ...
  run_or_fail "CI-PKGTEST-001" "Restore package-backed tests from package" dotnet restore "${package_tests_project}" --configfile "${package_tests_nuget_config}" -p:PackageBackedVersion="${package_version}" -p:RestoreLockedMode=false -v minimal

...
- `pull_request`/`push`: technische Qualitätsjobs (`preflight`, `version-policy`, `build`, `api-contract`, `pack`, `consumer-smoke`, `package-backed-tests`, `security-nuget`, `tests-bdd-coverage`, `summary`)
...
### Job: version-policy (fail-closed)

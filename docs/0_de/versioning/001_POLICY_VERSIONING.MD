<!-- LANG_SWITCH:BEGIN -->
[DE](001_POLICY_VERSIONING.MD) | [EN](../../1_en/versioning/001_POLICY_VERSIONING.MD)
<!-- LANG_SWITCH:END -->
...
## 2. Geltungsbereich
- Release-Tags `vX.Y.Z` (optional `-prerelease`) als einzige SSOT-Quelle für Paketversionen.
- PR-Versioning-Guard und Auto-Labeling in CI.

## 3. Begriffe
- SSOT: Single Source of Truth für Versioninformation.
- Breaking Change: Änderung, die bestehende Integrationen ohne Anpassung brechen kann.

## 4. SemVer-Regeln (verbindlich)
- `MAJOR`
  - öffentliche API entfernt/umbenannt
  - inkompatibles Standardverhalten
- `MINOR`
  - neue abwärtskompatible Features oder API-Erweiterungen
- `PATCH`
  - Fixes, Refactorings ohne Verhaltensbruch, Tests, Doku, CI, Tooling
- `NONE`
  - reine Metadatenänderungen ohne Versionsauswirkung
- Pre-Release-Tags sind erlaubt:
  - `vX.Y.Z-<label>` wird exakt als NuGet-Version `X.Y.Z-<label>` übernommen.
...
| Metadaten-only | mechanische Änderung ohne Laufzeitwirkung | NONE |
...
- Target-Feed für Online-Installierbarkeit: NuGet.org.
...
- `tools/versioning/check-versioning.sh` bleibt für Deterministik der `required`-Klassifikation im PR-Labeling.
...
## 9. Prioritätsregel
Primary-Priorität ist fix:
`breaking > feature > fix > refactor > ci > test > docs > tooling > chore`

## 10. Zugehörige SSOT-Dokumente
...
## 11. Qodana
Qodana ist zusätzliche Static-Analysis und ersetzt keine CI-Quality-Gates für Build/Test/Coverage/Versioning.

## 12. Dependency-Upgrade-Policy (Canary)
- Patch-/Minor-Upgrades für risikoreiche Dependencies werden über den Canary-Workflow vorab verifiziert:
...
- Major-Upgrades (`X+1.0.0`) sind für alle Canary-Dependencies nicht automatisch:

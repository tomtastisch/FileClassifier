<!-- LANG_SWITCH:BEGIN -->
[DE](005_POLICY_NAMING.MD) | [EN](../../1_en/governance/005_POLICY_NAMING.MD)
<!-- LANG_SWITCH:END -->
...
Änderungen am Namen erfolgen ausschliesslich durch Update dieser SSOT-Datei plus begleitende Migration.
...
- Einträge in `deprecated_package_ids` dürfen nicht als kanonische Paketreferenz verwendet werden
...
2. `README.md` und definierte Nutzungsdokumente
   ...
   - IDs aus `deprecated_package_ids` dürfen dort nicht als Install-Snippet auftauchen
3. CI/Release
   - `tools/ci/bin/run.sh` erzwingt in `run_pack` exakt `package_id`
   - `.github/workflows/release.yml` läuft einen Naming-Gate vor Pack/Publish
...
- `tools/ci/check-naming-snt.sh`
  - liest SSOT
  - prüft Projektmetadaten, Doku-Referenzen und Workflow/CI-Stellen
...
2. Legacy-Migration vorbereiten/ausführen:
...
6. CI-Gates (`naming-snt`, `pack`, `summary`) müssen grün sein.

Hinweis:
- Falls lokale NuGet-CLI kein `deprecate` unterstützt, erfolgt Deprecation im nuget.org UI (Manage Package -> Deprecation).
...
## 7. Nicht-Ziele
- Keine Hard-Delete-Annahme für nuget.org.
- Keine parallelen "empfohlenen" Paketnamen in Doku oder CI.
- Keine konfigurationslosen Sonderfälle ausserhalb SSOT.

## 8. Post-Merge Release-Kontrolle
Nach Merge/Tag-Release gilt verpflichtend:
1. Release-Workflow für den Tag laufen lassen.
